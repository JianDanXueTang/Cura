...
UM.MainWindow
{
    ...
    title: catalog.i18nc("@title:window","Udi-link");
    ...
    {	
        ...
        Item
        {
			...
			//注释，DropArea：QML的拖动的组件。2017.10.25，：寻找关闭主界面print的方法
            DropArea
            {
				anchors.fill: parent;
                onDropped:
            ...
            Image
            {
				...
				//书签，尝试修改主界面左下角的cura图标（对应文件为logo.svg）为UDI-Link.svg(√)
				//注释掉下面3行，或将代码中的“logo”或文件名改为非logo，可以不显示。替换的话UDI-Link.svg命名为logo.svg
                //不显示
            ...
            Sidebar
            {
                ...
                //★书签，修改主界面右侧参数控制模块的宽度（√，粗略成功，还有许多需要调整）
                //width: UM.Theme.getSize("sidebar").width;
                width: UM.Theme.getSize("sidebar").width/1.5;        
